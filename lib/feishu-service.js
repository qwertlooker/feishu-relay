...
function createFeishuService(client) {
  ...
  return {
    ...
    async uploadResource({ kind, buffer, fileName, fileType = 'stream' }) {
      if (kind === 'image') {
      ...
      return { key: result.file_key, msgType };
    },
    ...
    async getMessageResource({ messageId, fileKey, type }) {
      try {
        ...
        // messageResource is intended for resources attached to received messages.
        // Resources sent by this bot remain downloadable through image/file.get.
        if (type === 'image') {
          return client.im.image.get({ path: { image_key: fileKey } });
        }
        return client.im.file.get({ path: { file_key: fileKey } });
      }
    },
