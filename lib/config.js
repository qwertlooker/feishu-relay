...
function loadConfig(env = process.env) {
  ...
  const maxMessagesPerChat = Number(env.MAX_MESSAGES_PER_CHAT || 500);
  if (!Number.isInteger(maxMessagesPerChat) || maxMessagesPerChat < 1 || maxMessagesPerChat > 10000) {
    throw new Error('MAX_MESSAGES_PER_CHAT 必须是 1-10000 之间的整数');
  }
  const maxChats = Number(env.MAX_CHATS || 100);
  if (!Number.isInteger(maxChats) || maxChats < 1 || maxChats > 1000) {
    throw new Error('MAX_CHATS 必须是 1-1000 之间的整数');
  }
